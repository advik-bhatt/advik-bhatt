...
// Build a semicolon-joined keyTimes string clamped to [0,1]
...
function buildSvg(theme) {
  ...
  // Commit squares: fly linearly from grid → map, pop-collapse on arrival
  const commitSquares = active.map((cell, i) => {
    ...
    // Stagger departures evenly across flight window so the wave looks continuous
    ...
    // 5-keyframe x/y: sit on grid → fly linearly → hold at map pos → reset
    // 5-keyframe width/height/rx: full dot → still full during flight → small map dot → reset
    // 6-keyframe opacity: normal → blink flash on arrival (the "collapse" moment) → hold → fade
  ...
  // Border glow layer (wide, faint — drawn via stroke-dashoffset for wire effect)
  ...
  // Faint circuit grid overlay — only visible during map phase, reinforces cyber aesthetic
...
// Light — "Blueprint":   off-white, forest green nodes, indigo circuit traces
const themes = {
  dark: {
    id: 'dark',
    bg: '#040d1b',
